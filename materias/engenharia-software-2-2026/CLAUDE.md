...
Esta pasta é o **entregável completo da disciplina** Engenharia de Software II (UNESP, 2026) — documentação
do projeto final sobre o **Sistema Unespão** (plataforma de pedidos personalizados estilo Subway/Spoleto para
a padaria Unespão).

**Contexto:** Esta pasta vive dentro de um repositório maior (`../../` — raiz) que organiza:
- **`../../produto/`** — conhecimento central e agnóstico de disciplina sobre o Sistema Unespão (business.md e architecture.md).
- **`../../materias/<nome-da-materia>/`** — entregáveis autocontidos por disciplina (hoje: esta pasta).

O objetivo central **desta entrega**, no momento, é produzir a **documentação do projeto final** exigida pela
disciplina.
...
Os caminhos abaixo são relativos a **esta pasta** (`materias/engenharia-software-2-2026/`):
...
slides-teoricos/
  slides-SCM-handout.pdf
  slides-interface-handout.pdf
  slides-padroes-handout-parte-1.pdf
  slides-padroes-handout-site-parte-1.pdf
...
- **`../../produto/business.md` e `../../produto/architecture.md`** — versões em Markdown,
  agnósticas de disciplina, do mesmo conhecimento contido nos PDFs acima. Use como
  leitura rápida/referência antes de mergulhar nos PDFs completos.

### Forma/Padrão — como documentar

- **`exercicios-referencia/Exercicio-ESII-padrao-decorador.docx.pdf`** — referência de como
  ...
  *modelo de forma* para a seção "Padrões de Projeto", não como conteúdo do
  ...
  projeto).
- **`exercicios-referencia/Exercicio-teste-unidade-Java.docx.pdf`** — referência de estratégia de
  teste (JUnit + Mockito + JaCoCo). Usar como *modelo de forma* para a seção de
  Especificação e Estratégia de Testes.
- **`slides-teoricos/slides-SCM-handout.pdf`** — Gestão de Configuração de Software (SCIs,
...
- **`slides-teoricos/slides-interface-handout.pdf`** — Regras de Ouro de Mandel, do's/don'ts
...
- **`slides-teoricos/slides-padroes-handout-parte-1.pdf`** e
  **`slides-teoricos/slides-padroes-handout-site-parte-1.pdf`** — praticamente o mesmo
...
- Nenhum dos slides acima é fonte de conteúdo do Sistema Unespão —
...
**Nunca misture:** conteúdo do sistema vem do Sistema-Unespao-ESII.pdf (+ `../../produto/business.md`/`architecture.md` + input do
